...
function generate(title, children, hint) {
	...
	indicators.push({
		...
		children: parser.parse(title).children[0].children
	})
...
const defaults = {
	...
	callouts: {
		note: {
			title: "Note",
			hint: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="callout-hint callout-hint-note"><path d="M12 8h.01M12 12v4"/><circle cx="12" cy="12" r="10"/></svg>`
		},
		commend: {
			title: "Success",
			hint: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="callout-hint callout-hint-commend"><path d="m8 12 2.7 2.7L16 9.3"/><circle cx="12" cy="12" r="10"/></svg>`
		},
		warn: {
			title: "Warning",
...
export default function remarkCalloutDirectives(userOptions = {}) {
	...
	const aliases = defu(options.aliases, Object.keys(defaults.callouts).reduce((a, v) => ({ ...a, [v]: v}), {}))
	return (tree) => {
		visit(tree, (node) => {
